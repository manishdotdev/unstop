...
import { mockInterviews, interviewCategories } from "../../data/mockInterviewsData"

export default function MockInterview() {
  const [activeCategory, setActiveCategory] = useState("All")
  const [selectedCompany, setSelectedCompany] = useState("All")
  const [searchQuery, setSearchQuery] = useState("")
  const companies = ["All", "TCS", "Accenture", "Amazon", "Microsoft", "Deloitte", "NVIDIA", "EY", "Fractal Analytics"]
  ...
  const filteredInterviews = mockInterviews.filter(interview => {
    ...
    const matchesSearch = searchQuery === "" || 
      ...
      interview.skills.some(skill => skill.toLowerCase().includes(searchQuery.toLowerCase()))
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto">
              Practice with AI-powered mock interviews for real-world feedback and improvement
            </p>
          </div>
          ...
          <div className="relative max-w-2xl mx-auto mb-8">
            ...
            <input
              type="text"
              placeholder="Search mock interviews, companies, skills..."
              value={searchQuery}
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
            <FilterButton label="Filters" count={3} />
...
function MockInterviewCard({ interview }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
      ...
      <div className={`h-24 bg-gradient-to-r ${interview.color} relative flex items-center px-5`}>
        <div className="w-14 h-14 rounded-xl border-4 border-white bg-white shadow-lg flex items-center justify-center">
          <img 
      ...
      <div className="p-5">
        ...
        <p className="text-sm text-slate-500 mb-4">{interview.company}</p>
        ...
        <div className="flex items-center justify-between text-[12px] text-slate-500 mb-4">
          <div className="flex items-center gap-1">
            ...
            {interview.attempts.toLocaleString()} attempts
          </div>
          <div>⭐ {interview.avgRating}</div>
        </div>
        ...
        {/* Interview Info */}
        <div className="flex items-center justify-between text-[12px] text-slate-500 mb-4">
          <span>{interview.duration} · {interview.format}</span>
        </div>

        {/* Action Button */}
        <Link
          to={`/mockinterview/${interview.id}`}
